...
## Правила переходов статусов сигнала — Sources-01

Статусы: `signal` → `topic` → `confirmed_shift`

### SIGNAL
- Присваивается сразу при создании.
- Достаточно 1 источника.
- Похожие записи склеиваются через semantic dedup.

### TOPIC

**Путь A (сильный):** `unique_sources_count >= 3` AND `active_days >= 30`
- Числовой переход без ожидания мягкого критика.

**Путь B (временной):** `unique_sources_count >= 2` AND `active_days >= 7`
- Переход в topic — но мягкий критик в dedup-шаге обязан подтвердить.
- Если критик отклоняет → возврат в signal.

### CONFIRMED_SHIFT

Числовой пре-фильтр (без него тема не попадает к критику):
- `unique_sources_count >= 3` AND `active_days >= 30`

Строгий критик (LLM, `confirm-megatrends`) смотрит на:

**Путь A (устойчивый сдвиг):** 5+ источников + 90+ дней + 2+ активных месяца + критик одобряет

**Путь B (долгосрочный):** 3+ источников + продолжительное время (60+ дней) + критик одобряет

### Важно
- 10 повторов из одного источника ≠ 10 независимым источникам.
- Volume-only path (только количество сигналов без диверсификации источников) **удалён** в Sources-01.
- Источник считается независимым по полю `source_name` (не домену, не агрегатору).
- Перепечатки одной новости склеиваются semantic dedup'ом до подсчёта источников.
