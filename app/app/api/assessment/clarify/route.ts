...
resolve РАЗРЕШЁН только если выполнены ВСЕ условия:
1. В ответе ровно один смысл — нет смеси нескольких действий или ролей
2. Есть конкретное действие или навык (не просто контекст или среда)
3. Ответ однозначно маппится на один тег — второй тег не подходит так же хорошо
4. Это не абстракция ("умею общаться", "чувствую людей", "умею договариваться" без деталей)

Примеры → clarify (done: false):
- "умею чувствовать людей" → абстракция, нет действия
- "салон красоты" → контекст, не навык
- "продавал и нанимал людей" → два разных смысла
- "умею договариваться" → непонятно с кем и о чём

Примеры → resolve (done: true):
- "продавал строительные материалы b2b, вёл переговоры с подрядчиками" → конкретно, один смысл
- "умею договариваться с поставщиками по цене" → конкретное действие + контекст

${forceResolve ? 'РАУНД 2: clarify запрещён — обязательно resolve с best-effort интерпретацией.' : 'По умолчанию: если есть сомнение — clarify.'}
