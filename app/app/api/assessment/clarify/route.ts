...
const QUESTION_CONTRACTS: Record<string, QuestionContract> = {
  ...
  q5: {
    goal: 'Определить рыночный доступ — конкретные отрасли и наличие контактов',
    validSignal: 'Отрасль + тип контакта: "строительные подрядчики Москвы, 30+ директоров", "база из 200 b2b-клиентов по логистике"',
    invalidExamples: '"с разными компаниями" (нет конкретики), "в бизнесе" (слишком широко)',
    rephrase: 'В какой отрасли у вас больше всего знакомых или бывших клиентов? Кто эти люди?',
  },
  q6: {
    goal: 'Выяснить конкретный канал выхода на первых покупателей',
    validSignal: 'Конкретный механизм: "рекомендации от бывших клиентов", "Telegram-канал на 2000 чел.", "LinkedIn с базой 500 контактов"',
    invalidExamples: '"через интернет" (нет механизма), "найду" (нет канала), "через знакомых" (слишком широко)',
    rephrase: 'Если нужно найти первого клиента уже на этой неделе — кому конкретно написали бы или позвонили?',
  },
  q7: {
    goal: 'Проверить наличие партнёров, поставщиков или аудитории для запуска',
    validSignal: 'Конкретный тип связи + масштаб: "поставщик, работаем 3 года", "партнёр-технарь готов к сотрудничеству", "аудитория 2000 чел."',
    invalidExamples: '"есть связи" (абстракция), "найду партнёра при необходимости" (нет реального контакта)',
    rephrase: 'Есть ли конкретный человек или компания, которые прямо сейчас могут помочь вам запуститься?',
  },
  q8: {
    goal: 'Проверить наличие тёплого рынка — конкретных людей с которыми есть история и доверие',
    validSignal: 'Конкретные люди/компании: "бывший клиент X с кем работали 2 года", "три директора из профессионального чата"',
    invalidExamples: '"найду кого-нибудь" (нет конкретики), "есть много знакомых" (нет имён/типов)',
    rephrase: 'Есть ли кто-то — бывший клиент, коллега, знакомый — кто мог бы первым купить что-то у вас?',
  },
  q9: {
